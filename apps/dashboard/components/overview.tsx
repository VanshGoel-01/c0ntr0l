...
export function Overview(props: OverviewProps) {
  ...
  return <div className="overview-view view-enter">
    ...
    <div className="overview-secondary">
      <section className="panel application-panel"><div className="panel-heading"><div><h2>Application usage</h2><p>Token and recorded cost distribution</p></div></div><div className="application-list">{applications.slice(0, 5).map((application) => <button key={application.name} onClick={() => setSelectedApplication(application.name)} type="button"><div><strong>{application.name}</strong><span>{application.runs} runs / {application.active} active</span></div><div><b>{formatTokens(application.tokens)}</b><span>${application.cost.toFixed(2)} spend</span></div><i data-alert={application.incidents > 0}>{application.incidents}</i><ArrowRight size={15} /></button>)}</div></section>
