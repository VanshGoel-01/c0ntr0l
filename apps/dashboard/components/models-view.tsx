...
export function ModelsView({ connection, error, executions, loadingPolicies, policies, savingKeys, onMode, onLimit }: ModelsViewProps) {
  ...
  return <div className="module-view view-enter"><div className="page-intro"><div><h2>Models</h2><p>Runtime health, observed usage, token limits, and response policy for zero-cost routes.</p></div><button className="secondary-button" disabled={loading} onClick={() => void refresh()} type="button"><RefreshCw className={loading ? "spin" : undefined} size={15} />Recheck</button></div>
    ...
    <section className="panel model-table-panel"><div className="panel-heading"><div><h2>Model policies</h2><p>Project rules are stored by the control plane and shared across every connected client.</p></div><span className="policy-seal"><ShieldCheck size={15} />{loadingPolicies ? "Loading rules" : "Project scoped"}</span></div>{error && <div className="error-row">Policy save failed: {error}</div>}<div className="table-scroll"><table className="model-table"><thead><tr><th>Provider</th><th>Model</th><th>Runs</th><th>Tokens</th><th>Token limit</th><th>Latency</th><th>Policy mode</th></tr></thead><tbody>{models.map((model) => { const policy = policies[model.key] ?? { mode: "observe" as PolicyMode, tokenLimit: null }; const left = policy.tokenLimit === null ? null : Math.max(0, policy.tokenLimit - model.tokens); const saving = savingKeys.has(model.key); return <tr key={model.key}><td>{model.provider}</td><td><div className="model-identity"><ModelLogo model={model.model} provider={model.provider} /><div><strong>{model.model}</strong><small>{saving ? "Saving policy" : `${model.active} active / ${model.completed} completed`}</small></div></div></td><td>{model.runs}</td><td>{formatTokens(model.tokens)}</td><td><label className="token-limit"><input aria-label={`${model.model} token limit`} defaultValue={policy.tokenLimit ?? ""} disabled={saving} key={`${model.key}-${policy.tokenLimit ?? "none"}`} min="1" onBlur={(event) => { const value = event.target.value ? Number(event.target.value) : null; if (value !== policy.tokenLimit) void onLimit(model.provider, model.model, value).catch(() => undefined); }} placeholder="No limit" type="number" /><span>{left === null ? "Unlimited" : `${formatTokens(left)} left`}</span></label></td><td>{formatDuration(model.averageLatencyMs)}</td><td><div className="policy-control" role="group" aria-label={`${model.model} policy mode`}>{(["observe", "warn", "block"] as PolicyMode[]).map((mode) => <button data-active={policy.mode === mode} disabled={saving} key={mode} onClick={() => void onMode(model.provider, model.model, mode).catch(() => undefined)} type="button">{mode}</button>)}</div></td></tr>; })}</tbody></table></div></section>
  </div>;
}
