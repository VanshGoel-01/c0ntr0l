...
export function BudgetsView({ executions, workspace, modelPolicies }: { executions: Execution[]; workspace: WorkspaceContext; modelPolicies: Record<string, ModelPolicy> }) {
  ...
  return <div className="module-view view-enter"><div className="page-intro"><div><h2>Budgets</h2><p>Observed usage compared with workspace budgets and model-level call limits.</p></div><span className="policy-seal"><ShieldCheck size={16} />Server enforced</span></div>
    ...
    <section className="panel utilization-panel"><div className="panel-heading"><div><h2>Budget utilization</h2><p>Current 24-hour usage</p></div></div>{primary ? <div className="utilization-grid"><article><div><span>Request limit</span><strong>{requestPercent?.toFixed(1) ?? "--"}%</strong></div><progress max="100" value={requestPercent ?? 0} /><p>{requests.toLocaleString()} used / {primary.maxRequests?.toLocaleString() ?? "Unlimited"}</p></article><article><div><span>Token limit</span><strong>{tokenPercent?.toFixed(1) ?? "--"}%</strong></div><progress max="100" value={tokenPercent ?? 0} /><p>{tokens.toLocaleString()} used / {primary.maxTokens?.toLocaleString() ?? "Unlimited"}</p></article><article><div><span>Cost limit</span><strong>{primary.maxCost === null ? "Unlimited" : `${percent(cost, primary.maxCost)?.toFixed(1)}%`}</strong></div><progress max="100" value={primary.maxCost ? percent(cost, primary.maxCost) ?? 0 : 0} /><p>{primary.maxCost === null ? `$${cost.toFixed(2)} recorded / No limit` : `$${cost.toFixed(2)} used / $${primary.maxCost.toFixed(2)}`}</p></article></div> : <div className="empty-state"><strong>No workspace policy</strong><span>Seed or configure a PostgreSQL budget policy to calculate utilization.</span></div>}</section>
    <section className="panel policy-table-panel"><div className="panel-heading"><div><h2>Workspace policies</h2><p>Read from control.budget_policies</p></div></div><div className="table-scroll"><table><thead><tr><th>Policy</th><th>Scope</th><th>Period</th><th>Mode</th><th>Requests</th><th>Tokens</th><th>Cost</th><th>Status</th></tr></thead><tbody>{workspace.budgets.map((budget) => <tr key={budget.id}><td><strong>{budget.name}</strong></td><td>{budget.scopeType}</td><td>{budget.periodType}</td><td><span className="mode-pill" data-mode={budget.mode}>{budget.mode}</span></td><td>{budget.maxRequests?.toLocaleString() ?? "--"}</td><td>{budget.maxTokens?.toLocaleString() ?? "--"}</td><td>{budget.maxCost === null ? "Unlimited" : `$${budget.maxCost.toFixed(2)}`}</td><td>{budget.isEnabled ? "Enabled" : "Paused"}</td></tr>)}</tbody></table></div></section>
