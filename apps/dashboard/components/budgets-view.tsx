...
export function BudgetsView({ executions, workspace, modelPolicies }: { executions: Execution[]; workspace: WorkspaceContext; modelPolicies: Record<string, ModelPolicy> }) {
  ...
  return <div className="module-view view-enter"><div className="page-intro"><div><h2>Budgets</h2><p>Observed usage compared with PostgreSQL policy limits and model-level review limits.</p></div><span className="policy-seal"><ShieldCheck size={16} />Zero-cost routes</span></div>
    ...
    <section className="panel model-budget-panel"><div className="panel-heading"><div><h2>Model limits</h2><p>Review policies saved from the Models page</p></div></div><div className="model-budget-list">{models.map((model) => { const policy = modelPolicies[model.key]; const used = percent(model.tokens, policy?.tokenLimit ?? null); return <article key={model.key}><div className="model-budget-identity"><ModelLogo model={model.model} provider={model.provider} /><div><strong>{model.model}</strong><span>{model.provider}</span></div></div><div><b>{formatTokens(model.tokens)}</b><span>{policy?.tokenLimit ? `${formatTokens(Math.max(0, policy.tokenLimit - model.tokens))} left` : "No limit"}</span></div><progress max="100" value={used ?? 0} /><span className="mode-pill" data-mode={policy?.mode ?? "observe"}>{policy?.mode ?? "observe"}</span></article>; })}</div></section>
  </div>;
}
