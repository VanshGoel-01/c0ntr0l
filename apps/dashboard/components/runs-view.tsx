...
export function RunsView({ executions, settings, requestedExecution, loadExecution, loadIntervention, modelPolicies, modelTargets, recoverExecution, cancelExecution, onSelectionHandled }: RunsViewProps) {
  ...
  if (detail) return <TraceDetails detail={detail} intervention={intervention} key={detail.id} modelPolicies={modelPolicies} modelTargets={modelTargets} onBack={() => { setDetail(null); setIntervention(null); }} onCancel={cancel} onOpenExecution={openExecution} onRecover={recover} settings={settings} />;
