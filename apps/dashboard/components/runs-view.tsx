...
import type { ConsoleSettings, Execution, ExecutionDetail, RuntimeIntervention, RuntimeRecoveryRequest, RuntimeRecoveryResult } from "@/lib/types";
...
export function RunsView({ executions, settings, requestedExecution, loadExecution, loadIntervention, recoverExecution, cancelExecution, onSelectionHandled }: RunsViewProps) {
  ...
  const modelTargets = useMemo(() => Array.from(new Map(executions.map((run) => [`${run.provider}:${run.model}`, { provider: run.provider, model: run.model }])).values()), [executions]);

  async function openRun(run: Execution) {
    setLoading(true); setLoadError(null);
    try {
      const [nextDetail, nextIntervention] = await Promise.all([loadExecution(run.id), loadIntervention(run.id)]);
  ...
  if (detail) return <TraceDetails detail={detail} intervention={intervention} modelTargets={modelTargets} onBack={() => { setDetail(null); setIntervention(null); }} onCancel={cancel} onRecover={recover} settings={settings} />;
