...
export function UsageChart({ data }: { data: ActivityChartData }) {
  ...
  const yFor = (value: number) => plot.bottom - (value / maxValue) * (plot.bottom - plot.top);
  ...
  return <section className="panel activity-panel" aria-labelledby="activity-heading">
    ...
    <div className="chart-summary"><div><span>Total {metric}</span><strong>{formatValue(total)}</strong></div><div><span>Average active interval</span><strong>{formatValue(average)}</strong></div><div><span>Peak interval</span><strong>{formatValue(maxValue)}</strong></div></div>
    <div className="activity-chart">
      <svg viewBox={`0 0 ${width} ${height}`} role="img"><title>{`${metric === "tokens" ? "Token usage" : "Execution count"} over the selected time range`}</title>
        {[0, .5, 1].map((ratio) => <g key={ratio}><line className="grid-line" x1={plot.left} x2={plot.right} y1={yFor(maxValue * ratio)} y2={yFor(maxValue * ratio)} /><text className="axis-value" x={plot.left - 9} y={yFor(maxValue * ratio) + 4} textAnchor="end">{formatValue(maxValue * ratio)}</text></g>)}
